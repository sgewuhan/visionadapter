 ...
 *
 * <p>
 * 2004.10.4 Changed by lld: 
 * (1) change NEW_LINE to CRLF as MIME required;
 * (2) add method: void encodeFromFileToFile(inFileName, outFileName);
 * (3) add method: void decodeFromFileToFile(inFileName, outFileName);
 * (4) add method: void encodeFromStreamToStream(is, os);
 * (5) add method: void decodeFromStreamToStream(is, os);
 * </p>
 * <p>
 * Change Log:
 * </p>
...
public class Base64 {
	...
	public static String encodeBytes(byte[] source, int options) {
		return encodeBytes(source, 1, source.length, options);
	} // end encodeBytes
	...
	public static String encodeBytes(byte[] source, int off, int len,
		...
		// Else, don't compress. Better not to use streams at all then.
			...
			byte[] outBuff = new byte[(len43) // Main 4:3
					...
					// Change made by lld 2004.10.4: change LF to CRLF
					// + (breakLines ? ( len43 / MAX_LINE_LENGTH ) : 0) ]; //
					// New lines
					+ (breakLines ? (len43 / MAX_LINE_LENGTH * 2) : 0)]; // New
																			// lines
			int d = 0;
			int e = 0;
			int len2 = len - 2;
	...
	// added by lld: 2004.10.4
	public static void encodeFromStreamToStream(java.io.InputStream is,
			java.io.OutputStream os) {
		Base64.InputStream bis = null;
		java.io.OutputStream bos = null;

		try {
			byte[] buffer = new byte[(int) (8192 * 4 / 3 + 8192
					/ MAX_LINE_LENGTH * 2 + 1)];
			int numBytes = 0;

			// Open a stream
			bis = new Base64.InputStream(is, Base64.ENCODE);
			bos = os;

			// Read until done
			while ((numBytes = bis.read(buffer, 0, 8192)) >= 0) {
				bos.write(buffer, 0, numBytes);
			}
		} // end try
		catch (java.io.IOException e) {
			System.err.println("Error encoding from stream to stream.");
			e.printStackTrace();
		} // end catch: IOException
	}

	// added by lld: 2004.10.4
	public static void encodeFromFileToFile(String inFileName,
			String outFileName) {
		java.io.InputStream is = null;
		java.io.OutputStream os = null;
		try {
			// Set up some useful variables
			java.io.File fileInput = new java.io.File(inFileName);
			java.io.File fileOutput = new java.io.File(outFileName);
			...
			// Open a stream
			is = new java.io.BufferedInputStream(new java.io.FileInputStream(
					fileInput));
			os = new java.io.BufferedOutputStream(new java.io.FileOutputStream(
					fileOutput));

			encodeFromStreamToStream(is, os);
			...
			System.err.println("Error encoding from file " + inFileName
					+ " to file " + outFileName);
			e.printStackTrace();
		} // end catch: IOException
		finally {
	...
	// added by lld: 2004.10.4
	public static void decodeFromStreamToStream(java.io.InputStream is,
			java.io.OutputStream os) {
		Base64.InputStream bis = null;
		java.io.OutputStream bos = null;
		...
		try {
			...
			// Open a stream
			bis = new Base64.InputStream(is, Base64.DECODE);
			bos = os;
			...
			System.err.println("Error decoding from stream to stream.");
			e.printStackTrace();
		} // end catch: IOException
	}

	// added by lld: 2004.10.4
	public static void decodeFromFileToFile(String inFileName,
			String outFileName) {
		Base64.InputStream bis = null;
		java.io.OutputStream bos = null;
		...
		try {
			...
			// Check for size of file
			if (fileInput.length() > Integer.MAX_VALUE) {
				System.err
						.println("File is too big for this convenience method ("
								+ fileInput.length() + " bytes).");
				return;
			} // end if: file too big for int index
			...
			// Open a stream
			bis = new Base64.InputStream(new java.io.BufferedInputStream(
					new java.io.FileInputStream(fileInput)), Base64.DECODE);
			bos = new java.io.BufferedOutputStream(
					new java.io.FileOutputStream(fileOutput));
			...
			System.err.println("Error decoding from file " + inFileName
					+ " to " + outFileName);
			e.printStackTrace();
		} // end catch: IOException
		finally {
	...
	public static class InputStream extends java.io.FilterInputStream {
		...
		public int read(byte[] dest, int off, int len)
			...
			for (i = 0; i < len; i++) {
				...
				// Changed by lld 2004.10.4: LF to CRLF
				if (b == '\n') {
					dest[off + i] = CRLF[0];
					dest[off + i + 1] = CRLF[1];
					i++;
