...
public class PMWebserviceImpl implements Serializable, RemoteAccess {
	...
	private static void createFolderEntry(String objectId) throws Exception {
		...
		// 首先得到PM Folder对象
		if (objectId == null) {
			throw new IllegalArgumentException("----Args PMID is Null");
		}
		ModelServiceFactory factory = ModelServiceFactory
				.getInstance(codebasePath);
		Debug.P("" + factory);
		FolderPersistence folderPersistence = factory
				.get(FolderPersistence.class);
		Debug.P("" + folderPersistence);
		...
		Debug.P("------>>>Folder:" + folderName + "  ContainerName:"
				+ containerName + "  isContainer=" + isContainer
				+ "  ParentFolderID=" + parent_wcId);
		Transaction tx = null;
		try {
			tx = new Transaction();
			tx.start();
			...
			if (iscreate) {
				// 如果父项是容器则在容器下创建文件夹
				if (isContainer) {
					Debug.P("-----Container----->>>Ready Create FolderPath: "
							+ (DEFAULT + "/" + folderName));
					String folderPath = DEFAULT + "/" + folderName;
					folderResult = FolderUtil.getFolderRef(folderPath,
							container, true);
				} else {
					Persistable persistable = GenericUtil
							.getPersistableByOid(parent_wcId);
					if (persistable != null && persistable instanceof Folder) {
						Folder parent_Folder = (Folder) persistable;
						folderResult = FolderUtil.createSubFolder(folderName,
								null, parent_Folder, null);
					}
				}
				if (folderResult != null) {
					String wc_oid = folderResult.getPersistInfo()
							.getObjectIdentifier().getStringValue();// OID
	 ...
	 * @return
	 */
	public static int modifyFolderEntry(String objectId, String newFolderName)
			throws Exception {
		int count = 0;
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			return (Integer) RemoteMethodServer.getDefault().invoke(method,
					klass, null, types, vals);
		} else {
			// 查询PM文件夹对象
			FolderPersistence folderPersistence = ModelServiceFactory
			...
			Debug.P("------->>Modify PM Folder:" + folder.getCommonName()
					+ " ;WC_PLMID=" + foid + "   ;ContainerName="
					+ containerName);
			try {
				SessionHelper.manager.setAdministrator();
				if (!StringUtils.isEmpty(foid)) {
					Persistable persistable = GenericUtil
							.getPersistableByOid(foid);
					if (persistable != null) {
						if (persistable instanceof Folder) {
							...
							Debug.P("----->>>OldFolderName:" + fName
									+ "  NewFolderName=" + folderName);
							if (!StringUtils.equals(fName, folderName)) {
								count = folderService.editFolder(folderPath,
										folderName, containerName);
								if (count > 0) {
									folder.doUpdate();
									Debug.P("------>>PM 更新 OldFolderName("
											+ fName + ") 成NewFolderName("
											+ folderName + ")Success!");
								}
							}
						}
			...
			if (!StringUtils.isEmpty(objectId)) {
				...
				try {
					if (StringUtils.isNotEmpty(foid)) {
						Debug.P("------Ready Delete FolderName:"
								+ folder.getCommonName()
								+ "  Windchill FolderId:" + foid);
						Persistable persistable = GenericUtil
								.getPersistableByOid(foid);
						if (persistable != null) {
							if (persistable instanceof Folder) {
								Folder folderObj = (Folder) persistable;
								String folderPath = folderObj.getFolderPath();
								count = folderService.deleteFolder(folderPath,
										containerName);
								if (count > 0) {
									folder.doRemove();
									Debug.P("----Remove PM Folder:"
											+ folder.getCommonName()
											+ " Success!");
								}
	 ...
	 * @throws Exception
	 */
	private static WTContainer checkWTContainerExist(String containerName)
			throws Exception {
	...
	public static int createWTDocumentEntry(String pm_docId) throws Exception {
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			SessionHelper.manager.setAuthenticatedPrincipal(VMUSER);
			DocumentPersistence docPersistance = ModelServiceFactory
			...
			if (doc != null) {
				String plmId = doc.getPersistInfo().getObjectIdentifier()
						.getStringValue();
				...
				pm_document.setSecondVid(Integer.valueOf(doc
						.getIterationIdentifier().getValue()));
				pm_document.doUpdate();// 閿熺潾闈╂嫹
				Debug.P("----->>>PM WCID:" + plmId + "  ;PM_Document:"
						+ pm_docId);
				return 1;
			}
			String containerName = pmfolder.getContainerName();
			Debug.P("----->>>>WC   Folder ID:" + wc_foid + "  是否为PM的容器文件夹:"
					+ isContainer + "  ;ContaienrName:" + containerName);
			try {
				Persistable persistable = null;
				WTContainer container = null;
				Debug.P("------>>>PM DOC_ID：" + pm_docId + "是否新建到Windchill="
						+ iscreate);
				if (iscreate) {
					if (!StringUtils.isEmpty(wc_foid)) {
						persistable = GenericUtil.getPersistableByOid(wc_foid);
					} else {
						container = GenericUtil
								.getWTContainerByName(containerName);
						persistable = GenericUtil.createNewPath(container);
					}
					if (persistable instanceof Folder) {
						Folder folder = (Folder) persistable;
						boolean isEmpty = StringUtils.isEmpty(pm_document
								.getPLMId());
						if (isEmpty) {
							Map<String, Object> ibas = new HashMap<String, Object>();
							setDocIBAValuesMap(ibas, pm_document);
							WTDocument document = DocUtils.createDocument(
									pm_document, null, VMUSER, ibas, folder);
							if (document != null) {
								if (isContainer) {
									GenericUtil.moveObject2Container(document,
											container, folder);
								} else {
									FolderUtil.changeFolder(document, folder);
								}
								String wcId = document.getPersistInfo()
										.getObjectIdentifier().getStringValue();
								pm_document.setPLMData(getObjectInfo(document));
								pm_document.setPLMId(wcId);
								pm_document.setMajorVid(document
										.getVersionIdentifier().getValue());
								pm_document.setSecondVid(Integer
										.valueOf(document
												.getIterationIdentifier()
												.getValue()));
								WriteResult result = pm_document.doUpdate();

								Debug.P("----->>>PM Return:(" + result.getN()
										+ ")Create WCID:" + wcId
										+ "  ;PM_Document:" + pm_docId);
								count = 1;
							}
						}
					}
				...
				e.printStackTrace();
				throw new Exception("Windchill 创建("
						+ pm_document.getCommonName() + ")文档失败!");
			} finally {
				SessionHelper.manager.setAuthenticatedPrincipal(VMUSER);
			}
		}
	...
	public static int updateWTDocumentEntry(String pm_id) throws Exception {
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			Debug.P("------>>>>Windchill中是否已经创建(" + pm_docName + "):"
					+ isCreated + "  Doc_Windchill:" + doc_id);
			try {
				SessionHelper.manager.setAuthenticatedPrincipal(VMUSER);
				// SessionHelper.manager.setAdministrator();
				if (isCreated) {
					...
					Persistable object = GenericUtil
							.getPersistableByOid(doc_id);
					if (object != null && object instanceof WTDocument) {
						WTDocument doc = (WTDocument) object;
						// WTDocument
						// doc=(WTDocument)getLastObjectByNum(object);
						...
						if (doc != null) {
							if (wt.vc.wip.WorkInProgressHelper.isCheckedOut(
									doc, wt.session.SessionHelper.manager
											.getPrincipal()))
								doc = (WTDocument) WorkInProgressHelper.service
										.checkin(doc, "update document Info");
						}

						pm_document.setPLMData(getObjectInfo(doc));
						pm_document.setPLMId(doc.getPersistInfo()
								.getObjectIdentifier().getStringValue());
						pm_document.setMajorVid(doc.getVersionIdentifier()
								.getValue());
						pm_document.setSecondVid(Integer.valueOf(doc
								.getIterationIdentifier().getValue()));
						pm_document.doUpdate();
						Debug.P("------>>>Update PM_DocumentName：" + pm_docName
								+ " Success!");
					}
				}
			} catch (Exception e) {
	 ...
	 * @throws Exception
	 */
	public static int deleteWTDocumentEntry(String objectType, String pm_docId,
			String deletAction) throws Exception {
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			checkNull(pm_docId);
			// DocumentPersistence
			// docPersistance=ModelServiceFactory.getInstance(codebasePath).get(DocumentPersistence.class);
			// PMDocument pm_document=docPersistance.get(new
			// ObjectId(pm_docId));
			// checkNull(pm_document);
			...
			try {
				...
				if (!StringUtils.isEmpty(pm_docId)) {
					// Persistable
					// object=GenericUtil.getPersistableByOid(wc_oid);
					if (objectType.equals("WTDocument")) {
						WTDocument doc = (WTDocument) searchWCObject(
								WTDocument.class, pm_docId, Contants.PMID);
						if (doc != null) {
						...
						WTPart part = (WTPart) searchWCObject(WTPart.class,
								pm_docId, Contants.PMID);
						if (part != null) {
						...
						EPMDocument epmdoc = (EPMDocument) searchWCObject(
								EPMDocument.class, pm_docId, Contants.PMID);
						if (epmdoc != null) {
	...
	public static int moveWTDocumentEntry(String pm_docId) throws Exception {
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			BasicDocument basic_object = ModelServiceFactory.getInstance(
					codebasePath).getBasicDocumentById(pm_docId);
			...
			try {
				...
				Debug.P("---->>Windchill Folder ID:" + wc_foid
						+ "  Windchill  ID:" + doc_wcId);
				if (!StringUtils.isEmpty(wc_foid)) {
					SessionHelper.manager.setAdministrator();
					Persistable object = GenericUtil
							.getPersistableByOid(doc_wcId);
					...
					if (object != null) {
						...
						if (isContainer) {
							...
							GenericUtil.moveObject2Container(object, container,
									folderObj);// 閿熺嫛璁规嫹閿熸枻鎷烽敓鏂ゆ嫹閿熸枻鎷风洰褰曢敓鏂ゆ嫹
							Debug.P("----->>>moveObject2Container:" + object);
						} else {
							if (StringUtils.isEmpty(wc_foid)) {
								throw new Exception("文档("
										+ basic_object.getCommonName()
										+ ")目标文件夹(" + folder.getCommonName()
										+ ")在Windchill系统不存在 ,无法执行移动操作!");
							}
							folderObj = (Folder) GenericUtil
									.getPersistableByOid(wc_foid);
							if (folderObj != null) {
								Debug.P("----->>>>Start Move Folder"
										+ folderObj.getName());
								object = FolderHelper.service.changeFolder(
										(FolderEntry) object, folderObj);
								PersistenceHelper.manager.refresh(object);
								Debug.P("----->>>>End Move("
										+ object.getPersistInfo()
												.getObjectIdentifier()
												.getStringValue() + ") Folder("
										+ folderObj.getFolderPath()
										+ ") Success!!");
							}
						}
						return 1;
	...
	private static Persistable getLastObjectByNum(Persistable object)
		...
		if (object instanceof WTPart) {
			...
			doc = (WTDocument) Utils.getWCObject(WTDocument.class,
					doc.getNumber());
			Debug.P("---->>>getLastObjectByNum:WTDocument=" + doc.getNumber()
					+ "---version-->" + doc.getVersionInfo().getIdentifier()
					+ "-----oid----->"
	 ...
	 * @throws WTException
	 */
	private static void setDocIBAValuesMap(Map<String, Object> ibas,
			PMDocument object) throws WTException {
	 ...
	 * @throws WTException
	 */
	private static Map<String, Object> getObjectInfo(Persistable object)
			throws Exception {
		...
		if (object != null) {
			if (object instanceof WTPart) {
				...
				// result.put(ConstanUtil.STATE,
				// epm.getLifeCycleState().getDisplay());
				result.put(ConstanUtil.CREATOR, epm.getCreator().getName());
				result.put(ConstanUtil.MODIFIER, epm.getModifier().getName());
				result.put(ConstanUtil.CREATEDATE, epm.getCreateTimestamp()
						.toString());
				result.put(ConstanUtil.MODIFYDATE, epm.getModifyTimestamp()
						.toString());
				result.put(ConstanUtil.DOWNLOAD_URL,
						GenericUtil.getPrimaryContentUrl(epm));
				...
				// result.put(ConstanUtil.STATE,
				// doc.getLifeCycleState().getDisplay());
				result.put(ConstanUtil.CREATOR, doc.getCreator().getName());
				result.put(ConstanUtil.MODIFIER, doc.getModifier().getName());
				// result.put(ConstanUtil.CREATEDATE,
				// doc.getCreateTimestamp().toString());
				// result.put(ConstanUtil.MODIFYDATE,
				// doc.getModifyTimestamp().toString());
				result.put(ConstanUtil.DOWNLOAD_URL,
						GenericUtil.getPrimaryContentUrl(doc));
				Map<String, Object> ibas = LWCUtil.getAllAttribute(doc);
				result.putAll(ibas);
	...
	public static String getObjectVersion(RevisionControlled object) {
		...
		String result = null;
		try {
			String version = VersionControlHelper.getVersionIdentifier(object)
					.getValue();
			String iteration = VersionControlHelper.getIterationIdentifier(
					object).getValue();
	...
	public static void changeRevision(String pm_id) throws Exception {
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			Class[] types = { String.class, };
			Object[] vals = { pm_id };
			RemoteMethodServer.getDefault().invoke(method, klass, null, types,
					vals);
		} else {
			// 获取PM文档对象
			BasicDocument basic_object = ModelServiceFactory.getInstance(
					codebasePath).getBasicDocumentById(pm_id);
			checkNull(basic_object);
	...
	private static WTValuedHashMap createWTFolder(WTCollection wtcol,
			WTValuedMap map) throws WTException {
		WTValuedHashMap localWTValuedHashMap = new WTValuedHashMap();
		Persistable[] persistables = wtcol.toArray(new Persistable[0]);
		for (Persistable persistable : persistables) {
			Folder folder = FolderHelper.service
					.getFolder((FolderEntry) persistable);

			localWTValuedHashMap.put(
					((ObjectReference) map.get(persistable)).getObject(),
					folder);
		}
		return localWTValuedHashMap;
	}
	...
	public static int changePhase(String pm_id) throws Exception {
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			if (!StringUtils.isEmpty(pm_id)) {
				BasicDocument object = ModelServiceFactory.getInstance(
						codebasePath).getBasicDocumentById(pm_id);
				String phase = object.getPhase();
				String wc_id = object.getPLMId();
				Debug.P("----Phase--->>>Windchill ID:" + wc_id
						+ "   ;Phase Value:" + phase);
				if (!StringUtils.isEmpty(wc_id)) {
					try {
						SessionHelper.manager.setAuthenticatedPrincipal(VMUSER);
						...
						if (StringUtils.isNotEmpty(phase)
							...
							IBAUtils iba_values = new IBAUtils(
									(IBAHolder) persistable);
	 ...
	 *            对象的PMID
	 * @param class1
	 *            对象的类型，(wtpart,wtdocument,epmdocument)
	 * @param class2
	 *            要查询的对象类型，(wtpart,wtdocument,epmdocument)
	 * @param flag
	 *            如果查询对象结构(BOM结构)，true为向下查询，false为向上查询
	 * @return 返回关联对象的PMID集合
	 * @throws Exception
	 */
	public static List<String> queryWCObjectByPM(String pmid, String class1,
			String class2, boolean flag) throws Exception {
		List<String> pmList = new ArrayList<String>();
		WTPart part = null;
		Debug.P("pmid-->" + pmid + "---class1--->" + class1 + "----class2--->"
				+ class2 + "----flag--->" + flag);
		...
		if (class1.equalsIgnoreCase("wtpart")) {
			...
			Debug.P("-searchWCObject-->>WTPart：" + part + "   Num:"
					+ part.getNumber());
			if (part != null) {
				Debug.P("----Search Part:" + part);
				part = PartUtils.getPartByNumber(part.getNumber());
				Debug.P(part.getNumber() + "-------" + part.toString()
						+ "-------" + pmid);
				// 根据部件查部件
			...
			WTDocument doc = (WTDocument) searchWCObject(WTDocument.class,
					pmid, Contants.PMID);
	 ...
	 */
	public static Object searchWCObject(Class class1, String ibavalue,
			String ibakey) throws WTException, RemoteException,
	...
	public static Object doSearch(Class class1, String ibavalue, String ibakey) {
		...
		try {
			obj = (Object) RemoteMethodServer.getDefault().invoke(method,
					klass, null, types, vals);
		...
		if (!RemoteMethodServer.ServerFlag) {
			try {
				...
				return (String) RemoteMethodServer.getDefault().invoke(
						"getRepresentationByPM",
						PMWebserviceImpl.class.getName(), null, aclass, aobj);
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
			...
			if (object instanceof EPMDocument) {
				...
				if (app == null)
					return null;
				dataHandler = ContentHelper.service.getDownloadURL(
	...
	public static String getRepresentation(String masterid) throws Exception {
		...
		if (!RemoteMethodServer.ServerFlag) {
			try {
				...
				return (String) RemoteMethodServer.getDefault().invoke(
						"getRepresentation", PMWebserviceImpl.class.getName(),
						null, aclass, aobj);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			if (persistable instanceof EPMDocument) {
				...
				EPMDocument epmDoc = EPMDocUtil.getEPMDoc(epmDocument
						.getNumber());
			...
			// Object object =
			// searchWCObject(EPMDocument.class,pmid,Contants.PMID);
			// if(object instanceof EPMDocument){
			// }
		}
		Debug.P(">>>dataHandler:" + dataHandler);
	...
	public static void main(String[] args) throws Exception {
		...
		// Debug.P("strs---------->"+strs);
		// }
		Object obj = doSearch(EPMDocument.class, "54ae5a978a8775297f95150a",
				Contants.PMID);
		Debug.P(obj);
	...
	private static WTDocument checkWTDocumentWrite2PM(String pmOid) {
		...
		List<Persistable> list = LWCUtil.getObjectByIBA(ibaValues,
				ConstanUtil.WTDOCTYPE);
