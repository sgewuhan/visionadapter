...
/**
 * 读取SPM的中间表的数据后，根据时代新材的业务需求， 在时代新材WC中创建，更新部件和文档并同步到时代新材的PM系统中
 * 
...
public class CsrSpmUtil implements RemoteAccess, Serializable {
	 ...
	 * @throws Exception
	 */
	public static String createNewPart(HashMap<String, String> baseMap,
			Map<String, Object> ibaMap, Map<String, String> configMap)
			throws Exception {
		if (baseMap == null || baseMap.size() == 0)
			throw new WTException("物料基础属性参数集合为空!");
		Debug.P("---createNewPart-----> " + baseMap.toString()
				+ " ibaMaps:-- -> " + ibaMap);
		Transaction tx = null;
		// 创建部件
		try {
			...
			Debug.P("------>ContainerName:" + containerName + " PartType: "
					+ partType + ";PartNum:" + strNumber);
			...
			// 根据物料类型获得文件夹对象
			Folder folder = getFolderByType(partType, containerName, strNumber,
					configMap);
			if (folder != null) {
				Debug.P("----->>>Folder  ID:"
			...
			Debug.P("------>>>Create WTPart:"
					+ wtpart.getPersistInfo().getObjectIdentifier()
							.getStringValue() + " Part FolderPath:("
					+ wtpart.getFolderPath() + ")Success!!!");

			wtpart = (WTPart) PersistenceHelper.manager.refresh(wtpart);
			// 设置生命周期状态为"已发布"(与株洲所一致??)
			GenericUtil.changeState((LifeCycleManaged) wtpart,
					SPMConsts.RELEASED);
			tx.commit();
			tx = null;
		} catch (IOException e) {
	 ...
	 * @param ibaMap
	 */
	public static void updatePartInfo(WTPart part, Map<String, String> baseMap,
			Map<String, Object> ibaMap) throws Exception {
		Debug.P("---updatePartInfo--->>>baseMap:" + baseMap + "  --->ibaMap"
				+ ibaMap);
		Transaction tx = null;
		try {
			tx = new Transaction();
	 ...
	 * @return
	 */
	public static Folder getFolderByType(String partType, String containerName,
			String partNum, Map<String, String> configMap) throws Exception {
		Debug.P("-------------->>>GetFolderInfo<->PartType:" + partType
				+ "  partNum=" + partNum);
		...
		try {
			...
			if (folderRoute != null) {
				WTContainer wtcontainer = GenericUtil
						.getWTContainerByName(containerName);
				folder = FolderUtil.getFolderRef(folderRoute, wtcontainer,
						false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			throw new Exception("根据物料类型获取对应的文件夹对象失败,请检查容器(" + containerName
					+ ")是否Windchill中存在指定的文件夹对象.");
		}
		return folder;
	}
	 ...
	 * @return
	 */
	private static String getFolderPathByNum(String partNum,
		...
		if (configMap != null && configMap.size() > 0) {
			for (Iterator<?> ite = configMap.keySet().iterator(); ite.hasNext();) {
				...
				if (key.startsWith(SPMConsts.FOLDER_SPLIT)) {
					String temp_key = key.substring(key.indexOf("_") + 1)
							.toUpperCase();
	 ...
	 * 
	 */
	public static String updatePartForIba(String partNo,
			Map<String, Object> ibaMap, String introduct) throws Exception {
		try {
			...
			if (wtpart != null) {
				...
				if (wtpart != null) {
					if (wt.vc.wip.WorkInProgressHelper.isCheckedOut(wtpart,
							wt.session.SessionHelper.manager.getPrincipal()))
						wtpart = (WTPart) WorkInProgressHelper.service.checkin(
								wtpart, "update WTPart IBA");
					Debug.P("---->>>IBA Update Success");
				}
			}
			return "操作成功";
	...
	public static String changePartName(WTPart wtpart, String name)
		...
		// return (String) RemoteMethodServer.getDefault().invoke(method,klass,
		// null, types, values);
		// }
		...
		try {
			if (wtpart != null && !StringUtils.equals(wtpart.getName(), name)) {// 名称不一致才修改
				tx = new Transaction();
				tx.start();
				Debug.P("---->>>changePartName OldName:" + wtpart.getName()
						+ " ;NewName:" + name);
				...
				tx.commit();
				tx = null;
				Debug.P("--->>ChangePartName:("
						+ wtpart.getPersistInfo().getObjectIdentifier()
								.getStringValue() + ")名称为(" + name
						+ ")Success!");
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @throws WTException
	 */
	public static WTDocument createNewDocument(HashMap<String, String> baseMap,
		...
		try {
			...
			WTContainer container = GenericUtil
					.getWTContainerByName(wtcontainer);
			...
			Folder folder = FolderUtil
					.getFolderRef(foldePath, container, false);
			String pmfoid = null;
			if (folder != null) {
	 ...
	 * @throws Exception
	 */
	private static void updateDocContent2PM(WTDocument doc, PMDocument pmdoc,
			ModelServiceFactory factory) throws Exception {
		if (doc != null) {
			try {
				...
				if (ins != null) {
					...
					if (fileProvider != null) {
						...
						if (object != null) {
							...
							pmdoc.setContentVault((ObjectId) (pmdoc
									.getContent().getFileData().get("_id")),
									CONTENTVAULT_FILE, fileName, factory
											.getDB(PM2), ins);
						}
					}
					ins.close();
	 ...
	 * 创建PM文档
	 */
	public static void createDoc2PM(WTDocument doc) throws Exception {
		if (doc != null) {
			Debug.P("----->>>DOC:" + doc.getName());
			ModelServiceFactory factory = ModelServiceFactory
					.getInstance(CODEBASE_);
			DocumentPersistence docPersistence = factory
					.get(DocumentPersistence.class);
			PMDocument pmdoc = docPersistence.newInstance();
			String docId = doc.getPersistInfo().getObjectIdentifier()
					.getStringValue();
			Folder folder = FolderHelper.getFolder(doc);
			if (folder == null)
				throw new Exception("Windchill Doc" + docId + " 对应的文件夹为空不存在!");
			String doc_foid = folder.getPersistInfo().getObjectIdentifier()
					.getStringValue();
			...
			pmdoc.setSecondVid(Integer.valueOf(doc.getIterationIdentifier()
					.getValue()));
			// 上传主内文档
			try {
				InputStream ins = DocUtils.doc2is(doc);
				if (ins != null) {
					String fileName = DocUtils.getPrimaryFileNameByDoc(doc);
					pmdoc.setContentVault(new ObjectId(), CONTENTVAULT_FILE,
							fileName, factory.getDB(PM2), ins);
				}
				ins.close();
				pmdoc.doInsert();
				// 加载文档权限
				...
				iba.setIBAValue(SPMConsts.PMID,
						pmdocId == null ? "" : pmdocId.toString());
				iba.updateIBAPart(doc);
				Debug.P("--createDoc2PM--->>>Set PM ID2 Winchill:(" + pmdocId
						+ ")Success!!!");
			} catch (IOException e) {
				e.printStackTrace();
	...
	public static WTDocument reviseDocument(WTDocument doc,
			HashMap<String, String> baseMap, Map<String, Object> ibaMap)
			throws WTException {
		Debug.P("---reviseDocument-->>para hashmap is " + baseMap
				+ " ibaMap is -> " + ibaMap);
		Transaction tx = null;
		...
		try {
			...
			DocumentPersistence docPersistence = factory
					.get(DocumentPersistence.class);
			String wcoid = doc.getPersistInfo().getObjectIdentifier()
					.getStringValue();
			...
			String updocId = doc.getPersistInfo().getObjectIdentifier()
					.getStringValue();
			String state = doc.getLifeCycleState().getDisplay();
	...
	public static void updateDescribedDocument(WTPart part, String tzNum)
			throws WTPropertyVetoException, Exception {

		Debug.P("-------->>updateDescribedDocument:" + part + "  ;tzNum(Doc):"
				+ tzNum);
		if (part != null) {
			part = (WTPart) GenericUtil.checkout(part);
			// 获得部件的描述文档
			QueryResult qrOldWTDocs = WTPartHelper.service
					.getDescribedByDocuments(part);
			Vector<Document> oldwtdocs = qrOldWTDocs.getObjectVectorIfc()
					.getVector();
			if (StringUtils.isNotEmpty(tzNum)) {
				WTDocument tzDoc = (WTDocument) GenericUtil
						.getObjectByNumber(tzNum);
				// 删除旧关联
				for (int i = 0; i < oldwtdocs.size(); i++) {
					...
					if (doc instanceof WTDocument) {
						...
						if (StringUtils.equals(tzNum, descDoc.getNumber())) {// 存在历史关系(保证编号唯一才可行)
							QueryResult res = PersistenceHelper.manager.find(
									WTPartDescribeLink.class, part,
									"describes", (WTDocument) doc);
							while (res.hasMoreElements()) {
								WTPartDescribeLink rule = (WTPartDescribeLink) res
										.nextElement();
				...
				if (tzDoc != null) {
					if (wt.vc.wip.WorkInProgressHelper.isCheckedOut(tzDoc,
							wt.session.SessionHelper.manager.getPrincipal()))
						tzDoc = (WTDocument) WorkInProgressHelper.service
								.checkin(tzDoc, "update document Info");
				}
			}
		}
	}
	...
	// * @throws IOException
	// */
	// public static WTDocument updatePrimaryForDoc(WTDocument doc, String
	// location)
	...
	// * @throws WTException
	// */
	// public static Folder getFolder(WTContainerRef containerRef, String
	// folder)
	// throws WTException {
	...
	// * @throws WTException
	// */
	// public static void setState(LifeCycleManaged lifecyclemanaged, String
	// state)
	...
	// //获取与part关联的技术规格书参考文档
	// Vector<WTDocument> vect =
	// TechDocCommand.getJSGGSFromPart(vectPart.get(j));
	// for(int i=0; i< vect.size();i++){
	...
	// * @throws Exception
	// */
	// public static void updateRepresentation(String partNumber, String
	// imgPath)
	// throws Exception {
	// // 获取表示法
	// WTPart part = null;
	 ...
	 * @param ibaMap
	 */
	public static void updateWTPartIBA(WTPart part, Map<String, Object> ibaMap)
			throws Exception {
		Debug.P("---->>>IBA Map:" + ibaMap);
		if (ibaMap != null) {
	...
	private static String STATE_PATH = "codebase" + File.separator + "ext"
			...
			+ File.separator + "state.properties";
	...
	@SuppressWarnings("rawtypes")
	public static Map<String, String> getWCMappingFieldBySPM() throws Exception {
		Properties prop = new Properties();
		Map<String, String> mappingMap = new HashMap<String, String>();
		String wthome = (String) (WTProperties.getLocalProperties())
				.getProperty("wt.home", "");
	 ...
	 * @throws Exception
	 */
	public static boolean matchFactory(String factorys) throws Exception {
		// 获取PM的工厂信息
		List<String> fac_res = getAllPMFactory();
